...
- `src/lambda_function.py` — the webhook entry point. Verifies the `X-Telegram-Bot-Api-Secret-Token` header against `WEBHOOK_SECRET_TOKEN`, then processes the update via `async with application:` (the same `Application` from `bot.app.build_application`, built once at module scope) — this is PTB's own recommended pattern for serverless: `initialize()`/`shutdown()` once per invocation rather than trying to persist an event loop across warm Lambda invocations.
- `scripts/build_lambda.py` — builds the deployment package into `infra/lambda_build/` (gitignored) via a plain `pip install --target`, no Docker. All three runtime dependencies and their full transitive trees are pure Python (no C extensions), so this works regardless of host OS/architecture — there was no need for `aws_lambda_python_alpha`'s Docker-based bundling. Run this before every `cdk deploy`.
...
- CDK's own dependencies (`aws-cdk-lib`, `constructs`) live in a separate `infra` `uv` dependency group (`uv run --group infra ...`), not in the bot's runtime dependencies — keeps the Lambda package (built from `pyproject.toml`'s main deps only) from being bloated with deployment tooling.
- Deploy flow: `cdk bootstrap` (one-time per account/region) → `uv run python scripts/build_lambda.py` → `cd infra && cdk deploy` → `PYTHONPATH=src uv run python scripts/set_webhook.py <function-url-from-deploy-output>`.
- Telegram only delivers updates via one mechanism at a time — once a webhook is registered, local long-polling (`bot.local`) will start throwing `Conflict` errors. That's expected, not a bug; it means the webhook took over. To go back to local dev, delete the webhook first (`curl https://api.telegram.org/bot<token>/deleteWebhook`).
