...
def game_roster(scope: str, month: str, date: str) -> list[dict]:
    """Every original squad member's status for `date`, in registration order:
    `registrant_id` is always the original registrant, `skipped` says whether
    they backed out, and `replacement_id` is who (if anyone) is currently
    covering their spot. Used to render the full squad in game cards — unlike
    attendees_for_date, a skipped-and-unfilled member still gets an entry."""
    roster = []
