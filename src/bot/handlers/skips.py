...
async def skip_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not db.is_registered(chat_id, month, user.id):
    ...
    available = [d for d in upcoming if db.get_skip(chat_id, d, user.id) is None]
    if not available:
        await update.effective_message.reply_text(
...
async def skip_pick_callback(
    ...
    if db.get_skip(chat_id, date_str, user.id) is not None:
        await query.answer("You've already skipped this game.")
        return

    db.add_skip(chat_id, date_str, user.id)
    await query.edit_message_text(
        f"You're marked as skipping {date_str}. Looking for a replacement..."
    )
    await query.answer()

    skipper = db.get_player(chat_id, user.id)
    await _offer_next(context.bot, chat_id, month, date_str, skipper)


async def _offer_next(
    bot, chat_id: int, month: str, date_str: str, skipper: dict
) -> None:
    ...
    candidate = db.get_player(chat_id, int(waitlist[0]["user_id"]))
    ...
    await bot.send_message(chat_id, text, entities=entities, reply_markup=keyboard)
...
async def replace_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    skip = db.get_skip(chat_id, date_str, skipper_id)
    if skip is None or skip["status"] != "open":
        await query.answer("This spot has already been filled.", show_alert=True)
        return

    if action == "decline":
        db.remove_waitlist_entry(chat_id, month, candidate_id)
        await query.edit_message_text(query.message.text + "\n\n(declined)")
        await query.answer()
        skipper = db.get_player(chat_id, skipper_id)
        await _offer_next(context.bot, chat_id, month, date_str, skipper)
        return

    db.remove_waitlist_entry(chat_id, month, candidate_id)
    db.set_skip_replaced(chat_id, date_str, skipper_id, candidate_id)

    skipper = db.get_player(chat_id, skipper_id)
    candidate = db.get_player(chat_id, candidate_id)
